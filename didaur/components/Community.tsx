...
const Community: React.FC<CommunityProps> = ({ onPointsUpdate, user, isDarkMode }) => {
  ...
  const [marketItems, setMarketItems] = useState<MarketplaceItem[]>(getMarketItems());
  ...
  const filteredAndSortedMarket = useMemo(() => {
    let result = marketItems.filter(m => {
    ...
    return result;
  }, [marketItems, activeCategory, searchQuery, minPrice, maxPrice, sortBy]);
  ...
  const handleLike = (id: string) => {
    ...
    const updated = updateUserPoints(5, 0, false);
    if (updated) onPointsUpdate(updated);
  };

  const handleOpenComments = (postId: string) => {
    setViewingCommentsPostId(postId);
    setCurrentComments(getPostComments(postId));
  };
  ...
  const handleAddComment = () => {
    ...
    const updatedUser = savePostComment(viewingCommentsPostId, comment);
  ...
  const confirmPurchase = () => {
    ...
    const result = purchaseMarketItem(itemToConfirm.id, itemToConfirm.price);
    
    if (result.success) {
      setNotification({ 
        message: 'Pembelian Berhasil!', 
        details: `-${itemToConfirm.price} XP dikurangi dari saldo kamu.`,
        type: 'success' 
      });
  ...
  const handleUpload = () => {
    if (!newPost.itemName || !newPost.description) return;
    ...
    const post: CommunityPost = {
      ...
      imageUrl: newPost.image || "https://picsum.photos/seed/waste/800/800",
      likes: 0,
      comments: 0,
      timestamp: Date.now(),
      pointsEarned: 250,
      ...
      price: newPost.isForSale ? newPost.price : undefined
    };
    ...
    const updated = updateUserPoints(250, 0, false);
    if (updated) onPointsUpdate(updated);
    setIsUploading(false);
    setNotification({ message: 'Karya Berhasil Dipublikasikan!', details: '+250 XP didapatkan!', type: 'success' });
  ...
  return (
    <div className="pb-24 animate-in fade-in duration-500">
      {/* Notification Toast */}
      {notification && (
        <div className={`fixed top-6 left-6 right-6 z-[300] p-5 rounded-[2rem] shadow-2xl animate-in slide-in-from-top-20 duration-500 flex items-center space-x-4 border-2 ${
        ...
        <div className="flex justify-between items-center">
          <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl space-x-1 shadow-inner">
             {['Inspirasi', 'Pasar'].map(v => (
               <button 
                key={v}
                onClick={() => { setActiveView(v as any); setSearchQuery(''); setShowFilters(false); }}
      ...
      {/* Item Detail View */}
      {selectedMarketItem && (
        <div className="fixed inset-0 z-[200] bg-white dark:bg-slate-950 overflow-y-auto animate-in slide-in-from-right duration-500 no-scrollbar">
           <div className="relative aspect-[4/5] w-full">
              <img src={selectedMarketItem.imageUrl} className="w-full h-full object-cover" />
              <button 
                onClick={() => setSelectedMarketItem(null)} 
                className="absolute top-6 left-6 w-12 h-12 bg-white/20 backdrop-blur-md border border-white/20 rounded-2xl flex items-center justify-center text-white"
              >
           ...
           <div className="p-8 -mt-10 relative bg-white dark:bg-slate-950 rounded-t-[3.5rem] space-y-8">
              <div className="space-y-4">
                 ...
                 <div className="flex items-center space-x-3">
                    ...
                    <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[9px] font-black text-slate-500 uppercase tracking-widest">{selectedMarketItem.materialTag}</span>
                 </div>
              </div>

              <div className="flex items-center space-x-4 p-5 bg-slate-50 dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                 ...
                 <div>
                    ...
                    <div className="flex items-center mt-1">
                       ...
                       <span className="text-[10px] text-slate-400 font-bold ml-1">4.9 (21 Penjualan)</span>
                    </div>
                 </div>
              </div>

              <div className="space-y-3">
                 <h3 className="text-lg font-black text-slate-900 dark:text-slate-100">Deskripsi Karya</h3>
                 <p className="text-sm text-slate-500 leading-relaxed font-medium">{selectedMarketItem.description}</p>
              </div>

              <div className="pb-32">
                 <button 
                   onClick={() => handleBuyItem(selectedMarketItem)}
                   className="w-full bg-green-600 text-white py-5 rounded-[2.5rem] font-black text-lg shadow-xl active:scale-95 transition-all"
                 >
                   DAPATKAN KARYA INI
                 </button>
              </div>
      ...
      {viewingCommentsPostId && (
        <div className="fixed inset-0 z-[250] flex items-end justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-[3rem] h-[80vh] flex flex-col animate-in slide-in-from-bottom-full duration-500 shadow-2xl">
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                 <h2 className="text-2xl font-black text-slate-900 dark:text-white">Komentar</h2>
                 <button onClick={() => setViewingCommentsPostId(null)} className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                   ...
                   <div key={comment.id} className="flex space-x-4">
                      ...
                      <div className="flex-1 space-y-1">
                         <div className="flex justify-between items-center">
                            ...
                            <span className="text-[9px] font-bold text-slate-400">{new Date(comment.timestamp).toLocaleTimeString()}</span>
                         </div>
                 ...
                 )) : <p className="text-center py-20 text-slate-400 text-xs font-black uppercase">Belum ada komentar.</p>}
              </div>
              <div className="p-8 border-t border-slate-100 dark:border-slate-800 flex items-center space-x-3">
                 <input 
                   type="text" 
                   className="flex-1 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl p-4 text-sm font-bold dark:text-white outline-none focus:ring-2 ring-green-500"
                   placeholder="Tulis pujian..."
                   value={newCommentText}
                   onChange={e => setNewCommentText(e.target.value)}
                   onKeyDown={e => e.key === 'Enter' && handleAddComment()}
                 />
                 <button onClick={handleAddComment} className="w-12 h-12 bg-green-600 text-white rounded-2xl flex items-center justify-center shadow-lg active:scale-90 transition-transform">
      ...
      {itemToConfirm && (
        <div className="fixed inset-0 z-[400] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 animate-in zoom-in duration-300">
              <div className="bg-green-600 p-8 text-center text-white">
                 <div className="w-20 h-20 bg-white/20 rounded-[2rem] flex items-center justify-center text-4xl mx-auto mb-4">🛍️</div>
                 <h2 className="text-2xl font-black">Konfirmasi</h2>
              </div>
              <div className="p-8 text-center space-y-6">
                 <p className="text-slate-500 dark:text-slate-400 font-medium text-sm leading-relaxed">
                   Apakah Anda yakin ingin membeli item ini seharga <span className="text-green-600 font-black">{itemToConfirm.price.toLocaleString()} XP</span>?
                 </p>
                 <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
                    <span className="text-[10px] font-black text-slate-400 uppercase">Saldo Kamu</span>
                    <span className={`text-sm font-black ${user.points < itemToConfirm.price ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`}>
                 ...
                 <div className="flex flex-col space-y-3">
                    <button 
                      ...
                      disabled={user.points < itemToConfirm.price}
                      className="w-full bg-green-600 text-white py-5 rounded-2xl font-black text-lg shadow-xl active:scale-95 disabled:opacity-50"
                    >
                      Konfirmasi
                    </button>
                    <button 
                      onClick={() => setItemToConfirm(null)} 
                      className="w-full bg-slate-100 dark:bg-slate-800 text-slate-500 py-5 rounded-2xl font-black text-lg active:scale-95"
                    >
                      Batal
                    </button>
                 </div>
      ...
      {isUploading && (
        <div className="fixed inset-0 z-[150] bg-black/60 backdrop-blur-md flex items-end justify-center">
           <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-[3.5rem] p-8 animate-in slide-in-from-bottom-full duration-500">
              <div className="flex justify-between items-center mb-6">
                 <h2 className="text-2xl font-black text-slate-900 dark:text-white">Bagikan Karya</h2>
                 <button onClick={() => setIsUploading(false)} className="w-10 h-10 bg-slate-100 rounded-xl text-slate-400">✕</button>
              </div>
              <div className="space-y-4 max-h-[70vh] overflow-y-auto no-scrollbar pb-8 px-1">
                 <div onClick={() => document.getElementById('up-img')?.click()} className="w-full aspect-video bg-slate-50 dark:bg-slate-800 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer overflow-hidden">
                 ...
                 <input type="text" placeholder="Judul Proyek..." className="w-full p-5 bg-slate-50 dark:bg-slate-800 dark:text-white rounded-2xl font-bold outline-none focus:ring-2 ring-green-500" value={newPost.itemName} onChange={e => setNewPost({...newPost, itemName: e.target.value})} />
                 <div className="bg-amber-50 dark:bg-amber-900/10 rounded-[2rem] p-6 space-y-4 border border-amber-100">
                    <div className="flex items-center justify-between">
                       <span className="text-sm font-black text-amber-800">Pasarkan Karya Ini?</span>
                       <input type="checkbox" className="w-6 h-6 rounded-lg text-green-600 focus:ring-green-500" checked={newPost.isForSale} onChange={e => setNewPost({...newPost, isForSale: e.target.checked})} />
                    </div>
                    {newPost.isForSale && <input type="number" placeholder="Harga (XP)..." className="w-full p-4 bg-white dark:bg-slate-900 dark:text-white rounded-xl font-black outline-none border border-amber-200" value={newPost.price || ''} onChange={e => setNewPost({...newPost, price: parseInt(e.target.value) || 0})} />}
                 </div>
                 <textarea placeholder="Ceritakan proses pembuatannya..." className="w-full p-5 bg-slate-50 dark:bg-slate-800 dark:text-white rounded-2xl font-medium text-sm outline-none focus:ring-2 ring-green-500 h-24" value={newPost.description} onChange={e => setNewPost({...newPost, description: e.target.value})} />
                 <button onClick={handleUpload} disabled={!newPost.itemName || !newPost.image} className="w-full bg-green-600 text-white py-5 rounded-[2.5rem] font-black shadow-xl active:scale-95 transition-all disabled:opacity-50">PUBLIKASIKAN SEKARANG</button>
              </div>
           </div>
        </div>
      )}
