...
// Added isDarkMode to props to fix TypeScript error in index.tsx
    ...
    <div className="pb-24 animate-in fade-in duration-500">
      <div className="p-8 pb-4 space-y-6 text-center">
         ...
         <div className={`${isDarkMode ? 'bg-slate-900' : 'bg-slate-100'} p-1.5 rounded-2xl flex space-x-1 shadow-inner`}>
            {['Mingguan', 'Seluruh Waktu'].map(tab => (
              <button
                key={tab}
      ...
      <div className="px-6 flex justify-center items-end space-x-4 h-72 mb-12 mt-4 relative">
        ...
        <div className="flex-1 flex flex-col items-center">
          <div className="relative group">
            ...
            <img src={top3[1]?.avatar} className="relative w-16 h-16 rounded-[1.5rem] border-4 border-white dark:border-slate-800 shadow-xl object-cover" />
          ...
          <div className={`w-full ${isDarkMode ? 'bg-slate-800/50' : 'bg-slate-200/50'} backdrop-blur-sm h-28 rounded-t-[2.5rem] mt-3 flex flex-col items-center justify-center border-t-2 ${isDarkMode ? 'border-slate-700' : 'border-slate-300'}`}>
             <span className={`${isDarkMode ? 'text-slate-100' : 'text-slate-800'} font-black text-lg`}>{top3[1]?.points.toLocaleString()}</span>
        ...
        <div className="flex-1 flex flex-col items-center">
          <div className="relative -mt-10 group">
            ...
            <img src={top3[0]?.avatar} className="relative w-24 h-24 rounded-[2rem] border-4 border-amber-400 shadow-2xl shadow-amber-200 object-cover" />
          ...
          <div className="w-full bg-gradient-to-b from-amber-400 to-amber-500 h-40 rounded-t-[3rem] mt-3 flex flex-col items-center justify-center shadow-xl shadow-amber-100 dark:shadow-none">
             <span className="text-white font-black text-2xl drop-shadow-md">{top3[0]?.points.toLocaleString()}</span>
        ...
        <div className="flex-1 flex flex-col items-center">
          <div className="relative group">
            ...
            <img src={top3[2]?.avatar} className="relative w-16 h-16 rounded-[1.5rem] border-4 border-white dark:border-slate-800 shadow-xl object-cover" />
          ...
          <div className={`w-full ${isDarkMode ? 'bg-orange-900/30' : 'bg-orange-100'} h-20 rounded-t-[2.5rem] mt-3 flex flex-col items-center justify-center border-t-2 ${isDarkMode ? 'border-orange-900/50' : 'border-orange-200'}`}>
             <span className={`${isDarkMode ? 'text-orange-400' : 'text-orange-700'} font-black text-lg`}>{top3[2]?.points.toLocaleString()}</span>
          ...
          {rest.map((player, idx) => (
            <div 
              ...
              className={`p-4 rounded-[2rem] flex items-center justify-between border shadow-sm transition-all hover:translate-x-1 ${
                player.id === currentUser?.id ? 'bg-green-600 border-green-700 shadow-green-100 dark:shadow-none' : (isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100')
              } animate-in fade-in slide-in-from-right-4`}
              style={{ animationDelay: `${idx * 50}ms` }}
            >
              <div className="flex items-center space-x-4">
                <span className={`font-black text-sm w-8 text-center ${player.id === currentUser?.id ? 'text-white/60' : 'text-slate-300 dark:text-slate-600'}`}>
                  #{player.rank}
                </span>
                <img src={player.avatar} className="w-12 h-12 rounded-2xl object-cover shadow-sm border-2 border-white dark:border-slate-800" />
                <p className={`font-black text-sm ${player.id === currentUser?.id ? 'text-white' : (isDarkMode ? 'text-slate-200' : 'text-slate-800')}`}>
                  {player.name} {player.id === currentUser?.id && '(Kamu)'}
                </p>
              </div>
              <div className="text-right">
                <p className={`font-black ${player.id === currentUser?.id ? 'text-white' : (isDarkMode ? 'text-green-400' : 'text-green-600')}`}>
                  {player.points.toLocaleString()}
                </p>
                <p className={`text-[9px] font-bold uppercase tracking-widest ${player.id === currentUser?.id ? 'text-green-200' : (isDarkMode ? 'text-slate-500' : 'text-slate-400')}`}>
                  Total XP
                </p>
              </div>
            </div>
      ...
      {currentUser && !top3.some(p => p.id === currentUser.id) && (
           ...
           <div className={`bg-slate-900 dark:bg-slate-800 rounded-[2rem] p-5 shadow-2xl flex items-center justify-between border border-white/5`}>
              <div className="flex items-center space-x-4">
                 <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center font-black text-white">
                    #{players.find(p => p.id === currentUser.id)?.rank || '?'}
                 </div>
